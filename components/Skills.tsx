    ...
    <section id="skills" className="py-24 bg-brand-black relative">
      <div className="container mx-auto px-6">
        ...
        <div className="mt-16 relative">
            {/* Decorative Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 h-3/4 bg-brand-primary/5 rounded-full blur-[100px]"></div>
            ...
            <div className="flex flex-wrap justify-center gap-8 max-w-5xl mx-auto relative z-10">
              {skills.map((skill, index) => (
                ...
                >
                  <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-brand-card border border-brand-border flex items-center justify-center group-hover:border-brand-primary group-hover:shadow-[0_0_20px_rgba(0,194,255,0.3)] transition-all duration-300">
                    <skill.icon size={32} className="text-brand-muted group-hover:text-brand-primary transition-colors" />
                  </div>
                  <span className="text-sm font-medium text-brand-text group-hover:text-white transition-colors">
                    {skill.name}
                  </span>
                </motion.div>
              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-20">
           {[
             ...
             <motion.div 
               ...
               className="bg-brand-card border border-brand-border p-6 rounded-2xl hover:border-brand-primary/30 transition-colors"
             >
               <h3 className="text-xl font-heading font-bold text-white mb-4">{category.title}</h3>
